...
class DashboardView(View):
    ...
    def get(self, request):
        ...
        today_sales = Sale.objects.filter(user=user, sale_date__range=(today_start, today_end)).aggregate(total=Sum('total_amount'))['total'] or Decimal('0')
...
class OfferDeleteView(View):
    ...
    def post(self, request, pk):
        ...
        offer_name = offer.name
        offer.delete()
        messages.success(request, 'Offer "{}" deleted successfully!'.format(offer_name))
        return redirect('customers:offer-list')
